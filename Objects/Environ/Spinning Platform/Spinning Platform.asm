...
spinningplatform.counter_mask		ds.w 1						; (2 bytes)
spinningplatform.flag			ds.b 1						; (1 byte)
...
Obj_SpinningPlatform:
		...
		lsl.w	#4,d0
		sub.w	d1,d0
		move.w	d0,spinningplatform.delay(a0)
...
.alt
		...
		add.w	d0,d0
		add.w	d2,d0
		move.w	d0,spinningplatform.timer(a0)
		move.w	d0,spinningplatform.delay(a0)					; set time delay
		andi.w	#$70,d1
		addi.w	#$10,d1
		add.w	d1,d1
		add.w	d1,d1
		subq.w	#1,d1
		move.w	d1,spinningplatform.counter_mask(a0)

		; init
		lea	ObjDat_SpinningPlatform(pc),a1
		jsr	(SetUp_ObjAttributes).w
...
.trapdoor
		...
		; wait
		subq.w	#1,spinningplatform.timer(a0)					; decrement timer
		bpl.s	.tanimate							; if time remains, branch
		move.w	spinningplatform.delay(a0),spinningplatform.timer(a0)
...
.spinningplatform
		...
		and.w	spinningplatform.counter_mask(a0),d0
		bne.s	.delay
		st	spinningplatform.flag(a0)

.delay
		tst.b	spinningplatform.flag(a0)
		beq.s	.sanimate

		; wait
		subq.w	#1,spinningplatform.timer(a0)
		bpl.s	.sanimate
		move.w	spinningplatform.delay(a0),spinningplatform.timer(a0)
		clr.b	spinningplatform.flag(a0)
...
; ---------------------------------------------------------------------------
		...
		; mappings
		include "Objects/Environ/Spinning Platform/Object Data/Anim - Spinning Platform.asm"
		include "Objects/Environ/Spinning Platform/Object Data/Anim - Trapdoor.asm"
		include "Objects/Environ/Spinning Platform/Object Data/Map - Spinning Platform.asm"
		include "Objects/Environ/Spinning Platform/Object Data/Map - Trapdoor.asm"
