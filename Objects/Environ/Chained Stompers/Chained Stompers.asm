...
Obj_ChainStomp:
		...
		andi.w	#$7F,d0
		beq.s	.plus
		addq.b	#1,chainstomp.switch(a0)					; set 1
...
.plus
		...
		tst.w	d0
		bne.s	.notzero
		move.b	d2,chainstomp.yoffset(a0)
;		clr.b	chainstomp.yoffset+1(a0)

.notzero
		move.b	d2,chainstomp.length(a0)
;		clr.b	chainstomp.length+1(a0)
		...
		; get ypos
		move.w	y_pos(a0),d0
		move.w	d0,chainstomp.origY(a0)

		; set cap ypos
		moveq	#-16,d1
		add.w	d0,d1
		move.w	d1,chainstomp.cap_yoffset(a0)

		; set chain ypos
		moveq	#-52,d1
		add.w	d0,d1
		move.w	d1,chainstomp.chain_yoffset(a0)
		...
		move.w	chainstomp.cap_yoffset(a0),(a1)+				; ypos
		move.w	#3,(a1)+							; frame (cap)

		; sub object 2
		move.w	x_pos(a0),(a1)+							; xpos
		move.w	chainstomp.chain_yoffset(a0),(a1)+				; ypos
		move.b	#4,1(a1)							; frame (chain) ; skip $22 byte (mapping_frame)

		; check
		tst.b	chainstomp.spike_flag(a0)
		bne.s	.action
		...
		moveq	#28,d0
		add.w	y_pos(a0),d0
		move.w	d0,chainstomp_spikes.spike_yoffset(a1)
...
.action
		...
		; sub objects 2 (chain)
		moveq	#0,d0
		move.b	chainstomp.yoffset(a0),d0
		move.w	d0,d1
		lsr.b	#5,d0
		addq.b	#4,d0
		move.b	d0,sub3_mapframe(a0)
		add.w	chainstomp.chain_yoffset(a0),d1
...
CStom_Type00:
		moveq	#0,d0
		move.b	chainstomp.switch(a0),d0					; move number 0 or 1 to d0
		...
		bpl.s	loc_B872
		cmpi.b	#$10,chainstomp.yoffset(a0)
		beq.s	loc_B8A0

loc_B872:
		tst.w	chainstomp.yoffset(a0)
		beq.s	loc_B8A0
...
loc_B892:
		subi.w	#$80,chainstomp.yoffset(a0)
		bhs.s	CStom_Restart
		clr.w	chainstomp.yoffset(a0)

loc_B8A0:
		clr.w	chainstomp.yvel(a0)
		bra.s	CStom_Restart
; ---------------------------------------------------------------------------

loc_B8A8:
		move.w	chainstomp.length(a0),d1
		cmp.w	chainstomp.yoffset(a0),d1
		beq.s	CStom_Restart
		move.w	chainstomp.yvel(a0),d0
		addi.w	#$70,chainstomp.yvel(a0)					; make object fall
		add.w	d0,chainstomp.yoffset(a0)
		cmp.w	chainstomp.yoffset(a0),d1
		bhi.s	CStom_Restart
		move.w	d1,chainstomp.yoffset(a0)
		clr.w	chainstomp.yvel(a0)						; stop object falling
		tst.b	render_flags(a0)						; object visible on the screen?
...
CStom_Restart:
		moveq	#0,d0
		move.b	chainstomp.yoffset(a0),d0
		add.w	chainstomp.origY(a0),d0
		move.w	d0,y_pos(a0)
		rts
; ---------------------------------------------------------------------------

CStom_Type01:
		tst.b	chainstomp.delay_flag(a0)
		beq.s	loc_B938
		tst.b	chainstomp.wait(a0)
		beq.s	loc_B902
		subq.b	#1,chainstomp.wait(a0)
		bra.s	CStom_Restart
; ---------------------------------------------------------------------------
...
loc_B91C:
		...
		clr.w	chainstomp.yvel(a0)
		clr.w	chainstomp.yoffset(a0)
		clr.b	chainstomp.delay_flag(a0)
		bra.s	CStom_Restart
; ---------------------------------------------------------------------------
...
loc_B938:
		...
		move.w	d1,chainstomp.yoffset(a0)
		clr.w	chainstomp.yvel(a0)						; stop object falling
		st	chainstomp.delay_flag(a0)
		move.b	#1*60,chainstomp.wait(a0)
		tst.b	render_flags(a0)						; object visible on the screen?
...
.main
		...
		move.b	chainstomp.yoffset(a1),d0
		add.w	chainstomp_spikes.spike_yoffset(a0),d0
		move.w	d0,y_pos(a0)
