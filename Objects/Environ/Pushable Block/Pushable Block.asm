...
movingblock.p2_status			ds.b 1						; Tails's status (1 byte)
...
.action
		tst.b	pushableblock.float_flag(a0)
...
.smove
		...
		move.w	pushableblock.copyXvel(a0),d0
		asr.w	#3,d0
		move.w	d0,x_vel(a0)
...
PushableBlock_EdgeFix:									; fix fall block
		...
		move.w	x_vel(a0),pushableblock.copyXvel(a0)
		clr.w	x_vel(a0)
...
PushableBlock_MovePush:									; push block normal
		...
		move.b	movingblock.p1_status(a0),d0					; p1 status
...
.main
		...
		; wait
		subq.b	#1,pushableblock.sound(a0)
		bpl.s	.endl
		move.b	#4,pushableblock.sound(a0)
...
; ---------------------------------------------------------------------------
		...
		; mappings
		include "Objects/Environ/Pushable Block/Object Data/Map - Pushable Block.asm"
